...
# Script's output is:
# ./$APP_NAME-$VERSION.xpi
# ./$APP_NAME-$VERSION.jar  (only if $KEEP_JAR=1)
# ./files -- the list of packaged files
#
# Note: It modifies chrome.manifest when packaging so that it points to
#       chrome/$APP_NAME-$VERSION.jar!/*
...
VERSION=`egrep "em:version" install.rdf | sed "s/.*>\([^<]*\)<.*/\\1/"`
JAR_FILE=$TMP_DIR/chrome/$APP_NAME-$VERSION.jar
...
if [ $CLEAN_UP = 0 ]; then
  # save the jar file
  mv $TMP_DIR/chrome/$APP_NAME-$VERSION.jar .
else
  rm ./files
fi
